...
<body class="h-screen flex flex-col flex-1 overflow-hidden " style="background: 12, 43, 121;">
    ...
    <div class="flex flex-1">
        ...
        <div class=" flex-1 flex flex-col" id="listview">
            ...
            <?php
                $res=$db->run("select lnf.*,p.name from lostnfound as lnf left join pictures as p on lnf.id=p.ref_id where lnf.type='found' and status='approved'");
                while($row=$res->fetch()){
                    ?>
                ...
                <div class="w-full sm:w-1/2 md:w-1/3 px-2">
                    <div class="relative bg-white rounded border">
                        <picture class="block bg-gray-200 border-b flex justify-center h-48">
                            <img class="block" src="uploads/found/<?=$row['id']?>/<?=$row['name']?>" alt="Card 1">
                        </picture>
                        <div class="p-4">
